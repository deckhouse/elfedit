...
- Replacing a section keeps its index, `sh_link`, `sh_info`, `sh_addr` and `sh_entsize`. A same-sized replacement whose existing offset satisfies the requested alignment is written in place when it does not overlap another file-backed section or the program-header table. Otherwise its data and a new section-header table are appended. The name table is not rewritten.
